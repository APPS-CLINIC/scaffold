...
and that "if IWA requires Tailwind, we enable Tailwind". The new
`useCustomIcon` primitive is styled utility-first, which forced the decision:
...
- New primitives (e.g. `useCustomIcon`) add no CSS files — easier to swap
  them for IWA components.
