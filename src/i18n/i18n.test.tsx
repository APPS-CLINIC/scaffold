...
describe('i18n', () => {
  ...
  it('pluralises the overdue message per language', async () => {
    const overdueDays = (count: number) => i18n.t('common.overdueDays', { count });
    const catalog = (messages: Record<MessageKey, string>, key: MessageKey, count: number) =>
      messages[key].replace('{{count}}', String(count));

    expect(overdueDays(1)).toBe(catalog(pl, 'common.overdueDays_one', 1));
    expect(overdueDays(2)).toBe(catalog(pl, 'common.overdueDays_few', 2));
    expect(overdueDays(5)).toBe(catalog(pl, 'common.overdueDays_many', 5));
    expect(overdueDays(22)).toBe(catalog(pl, 'common.overdueDays_few', 22));

    await i18n.changeLanguage('en');
    expect(overdueDays(1)).toBe(catalog(en, 'common.overdueDays_one', 1));
    expect(overdueDays(3)).toBe(catalog(en, 'common.overdueDays_other', 3));
  });
});
