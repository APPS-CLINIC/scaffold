...
describe('DataPanel', () => {
  ...
  it('renders configured fields through IWA definition lists and supports formatting', () => {
    ...
    expect(screen.getByText('AAA').closest('dl')).toHaveTextContent('Rating');
  ...
  it('keeps richer-value geometry when the value is empty', () => {
    ...
    const ratingRow = screen.getByText('Rating').closest('dl');
    expect(ratingRow).toHaveTextContent('—');
    expect(ratingRow?.querySelector('dd span')).toHaveClass('h-6');
  });
